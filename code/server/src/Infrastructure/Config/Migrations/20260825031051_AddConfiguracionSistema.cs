using System;
using Microsoft.EntityFrameworkCore.Migrations;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace IMT_Reservas.Server.src.Infrastructure.Config.Migrations
{
    public partial class AddConfiguracionSistema : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropIndex(
                name: "IX_audit_logs_admin_carnet_timestamp",
                table: "audit_logs");

            migrationBuilder.DropIndex(
                name: "IX_audit_logs_entidad_entidad_id",
                table: "audit_logs");

            migrationBuilder.DropColumn(
                name: "tiempo_max_prestamo",
                table: "equipos");

            migrationBuilder.AddColumn<bool>(
                name: "bloqueado",
                table: "usuarios",
                type: "boolean",
                nullable: false,
                defaultValue: false);

            migrationBuilder.AddColumn<string>(
                name: "motivo_bloqueo",
                table: "usuarios",
                type: "text",
                nullable: true);

            migrationBuilder.AddColumn<string>(
                name: "destino_prestamo",
                table: "prestamos",
                type: "character varying(50)",
                maxLength: 50,
                nullable: false,
                defaultValue: "Universidad");

            migrationBuilder.AddColumn<int>(
                name: "id_carrera",
                table: "prestamos",
                type: "integer",
                nullable: true);

            migrationBuilder.AddColumn<string>(
                name: "nombre_materia",
                table: "prestamos",
                type: "character varying(255)",
                maxLength: 255,
                nullable: true);

            migrationBuilder.AddColumn<bool>(
                name: "recordatorio_enviado",
                table: "prestamos",
                type: "boolean",
                nullable: false,
                defaultValue: false);

            migrationBuilder.AddColumn<int>(
                name: "tiempo_max_prestamo_dias",
                table: "grupos_equipos",
                type: "integer",
                nullable: false,
                defaultValue: 7);

            migrationBuilder.AlterColumn<DateOnly>(
                name: "fecha_ingreso_equipo",
                table: "equipos",
                type: "date",
                nullable: false,
                defaultValue: new DateOnly(2026, 8, 25),
                oldClrType: typeof(DateOnly),
                oldType: "date",
                oldDefaultValue: new DateOnly(2026, 6, 17));

            migrationBuilder.AddColumn<bool>(
                name: "estado_eliminado",
                table: "audit_logs",
                type: "boolean",
                nullable: false,
                defaultValue: false);

            migrationBuilder.CreateTable(
                name: "avisos_disponibilidad",
                columns: table => new
                {
                    id_aviso = table.Column<int>(type: "integer", nullable: false)
                        .Annotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn),
                    carnet_usuario = table.Column<string>(type: "character varying(20)", maxLength: 20, nullable: false),
                    id_grupo_equipo = table.Column<int>(type: "integer", nullable: false),
                    fecha = table.Column<DateTime>(type: "timestamp without time zone", nullable: false),
                    cantidad = table.Column<int>(type: "integer", nullable: false),
                    notificado = table.Column<bool>(type: "boolean", nullable: false),
                    fecha_creacion = table.Column<DateTime>(type: "timestamp without time zone", nullable: false),
                    estado_eliminado = table.Column<bool>(type: "boolean", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_avisos_disponibilidad", x => x.id_aviso);
                });

            migrationBuilder.CreateTable(
                name: "comentarios_equipos",
                columns: table => new
                {
                    id_comentario_equipo = table.Column<int>(type: "integer", nullable: false)
                        .Annotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn),
                    id_grupo_equipo = table.Column<int>(type: "integer", nullable: false),
                    id_comentario_padre = table.Column<int>(type: "integer", nullable: true),
                    carnet_usuario = table.Column<string>(type: "character varying(20)", maxLength: 20, nullable: false),
                    contenido = table.Column<string>(type: "character varying(1024)", maxLength: 1024, nullable: false),
                    fecha_creacion = table.Column<DateTime>(type: "timestamp without time zone", nullable: false),
                    likes = table.Column<int>(type: "integer", nullable: false, defaultValue: 0),
                    liked_by = table.Column<string>(type: "text", nullable: false, defaultValue: ""),
                    estado_eliminado = table.Column<bool>(type: "boolean", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_comentarios_equipos", x => x.id_comentario_equipo);
                    table.ForeignKey(
                        name: "FK_comentarios_equipos_comentarios_equipos_id_comentario_padre",
                        column: x => x.id_comentario_padre,
                        principalTable: "comentarios_equipos",
                        principalColumn: "id_comentario_equipo",
                        onDelete: ReferentialAction.Restrict);
                    table.ForeignKey(
                        name: "FK_comentarios_equipos_grupos_equipos_id_grupo_equipo",
                        column: x => x.id_grupo_equipo,
                        principalTable: "grupos_equipos",
                        principalColumn: "id_grupo_equipo",
                        onDelete: ReferentialAction.Cascade);
                    table.ForeignKey(
                        name: "FK_comentarios_equipos_usuarios_carnet_usuario",
                        column: x => x.carnet_usuario,
                        principalTable: "usuarios",
                        principalColumn: "carnet",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateTable(
                name: "configuraciones_sistema",
                columns: table => new
                {
                    id_configuracion = table.Column<int>(type: "integer", nullable: false)
                        .Annotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn),
                    monto_minimo_contrato = table.Column<decimal>(type: "numeric(18,2)", nullable: false),
                    horario_inicio_minutos = table.Column<int>(type: "integer", nullable: false),
                    horario_fin_minutos = table.Column<int>(type: "integer", nullable: false),
                    nombre_jefe_carrera = table.Column<string>(type: "character varying(255)", maxLength: 255, nullable: false),
                    firma_jefe_carrera_base64 = table.Column<string>(type: "text", nullable: false),
                    tiempo_minimo_reserva_minutos = table.Column<int>(type: "integer", nullable: false),
                    tiempo_recordatorio_previo_minutos = table.Column<int>(type: "integer", nullable: false),
                    minutos_gracia_atraso = table.Column<int>(type: "integer", nullable: false),
                    estado_eliminado = table.Column<bool>(type: "boolean", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_configuraciones_sistema", x => x.id_configuracion);
                });

            migrationBuilder.CreateTable(
                name: "notificaciones",
                columns: table => new
                {
                    id_notificacion = table.Column<int>(type: "integer", nullable: false)
                        .Annotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.IdentityByDefaultColumn),
                    carnet_usuario = table.Column<string>(type: "character varying(20)", maxLength: 20, nullable: false),
                    tipo = table.Column<string>(type: "character varying(50)", maxLength: 50, nullable: false),
                    titulo = table.Column<string>(type: "text", nullable: false),
                    contenido = table.Column<string>(type: "text", nullable: true),
                    detalle = table.Column<string>(type: "text", nullable: true),
                    leido = table.Column<bool>(type: "boolean", nullable: false),
                    fecha_envio = table.Column<DateTime>(type: "timestamp without time zone", nullable: false),
                    estado_eliminado = table.Column<bool>(type: "boolean", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_notificaciones", x => x.id_notificacion);
                });

            migrationBuilder.InsertData(
                table: "configuraciones_sistema",
                columns: new[] { "id_configuracion", "estado_eliminado", "firma_jefe_carrera_base64", "horario_fin_minutos", "horario_inicio_minutos", "minutos_gracia_atraso", "monto_minimo_contrato", "nombre_jefe_carrera", "tiempo_minimo_reserva_minutos", "tiempo_recordatorio_previo_minutos" },
                values: new object[] { 1, false, "data:image/png;base64,", 1080, 480, 15, 2000m, "Job Angel Ledezma Dr.Ing", 30, 30 });

            migrationBuilder.CreateIndex(
                name: "IX_prestamos_id_carrera",
                table: "prestamos",
                column: "id_carrera");

            migrationBuilder.CreateIndex(
                name: "unique_codigo_ucb",
                table: "equipos",
                column: "codigo_ucb",
                unique: true);

            migrationBuilder.CreateIndex(
                name: "IX_audit_logs_admin_carnet_timestamp_estado_eliminado",
                table: "audit_logs",
                columns: new[] { "admin_carnet", "timestamp", "estado_eliminado" });

            migrationBuilder.CreateIndex(
                name: "IX_audit_logs_entidad_accion_timestamp_estado_eliminado",
                table: "audit_logs",
                columns: new[] { "entidad", "accion", "timestamp", "estado_eliminado" });

            migrationBuilder.CreateIndex(
                name: "IX_audit_logs_entidad_entidad_id_estado_eliminado",
                table: "audit_logs",
                columns: new[] { "entidad", "entidad_id", "estado_eliminado" });

            migrationBuilder.CreateIndex(
                name: "IX_avisos_disponibilidad_notificado_estado_eliminado",
                table: "avisos_disponibilidad",
                columns: new[] { "notificado", "estado_eliminado" });

            migrationBuilder.CreateIndex(
                name: "IX_comentarios_equipos_carnet_usuario",
                table: "comentarios_equipos",
                column: "carnet_usuario");

            migrationBuilder.CreateIndex(
                name: "IX_comentarios_equipos_id_comentario_padre_estado_eliminado",
                table: "comentarios_equipos",
                columns: new[] { "id_comentario_padre", "estado_eliminado" });

            migrationBuilder.CreateIndex(
                name: "IX_comentarios_equipos_id_grupo_equipo_fecha_creacion_estado_e~",
                table: "comentarios_equipos",
                columns: new[] { "id_grupo_equipo", "fecha_creacion", "estado_eliminado" });

            migrationBuilder.CreateIndex(
                name: "IX_notificaciones_carnet_usuario_leido_estado_eliminado",
                table: "notificaciones",
                columns: new[] { "carnet_usuario", "leido", "estado_eliminado" });

            migrationBuilder.AddForeignKey(
                name: "FK_prestamos_carreras_id_carrera",
                table: "prestamos",
                column: "id_carrera",
                principalTable: "carreras",
                principalColumn: "id_carrera");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropForeignKey(
                name: "FK_prestamos_carreras_id_carrera",
                table: "prestamos");

            migrationBuilder.DropTable(
                name: "avisos_disponibilidad");

            migrationBuilder.DropTable(
                name: "comentarios_equipos");

            migrationBuilder.DropTable(
                name: "configuraciones_sistema");

            migrationBuilder.DropTable(
                name: "notificaciones");

            migrationBuilder.DropIndex(
                name: "IX_prestamos_id_carrera",
                table: "prestamos");

            migrationBuilder.DropIndex(
                name: "unique_codigo_ucb",
                table: "equipos");

            migrationBuilder.DropIndex(
                name: "IX_audit_logs_admin_carnet_timestamp_estado_eliminado",
                table: "audit_logs");

            migrationBuilder.DropIndex(
                name: "IX_audit_logs_entidad_accion_timestamp_estado_eliminado",
                table: "audit_logs");

            migrationBuilder.DropIndex(
                name: "IX_audit_logs_entidad_entidad_id_estado_eliminado",
                table: "audit_logs");

            migrationBuilder.DropColumn(
                name: "bloqueado",
                table: "usuarios");

            migrationBuilder.DropColumn(
                name: "motivo_bloqueo",
                table: "usuarios");

            migrationBuilder.DropColumn(
                name: "destino_prestamo",
                table: "prestamos");

            migrationBuilder.DropColumn(
                name: "id_carrera",
                table: "prestamos");

            migrationBuilder.DropColumn(
                name: "nombre_materia",
                table: "prestamos");

            migrationBuilder.DropColumn(
                name: "recordatorio_enviado",
                table: "prestamos");

            migrationBuilder.DropColumn(
                name: "tiempo_max_prestamo_dias",
                table: "grupos_equipos");

            migrationBuilder.DropColumn(
                name: "estado_eliminado",
                table: "audit_logs");

            migrationBuilder.AlterColumn<DateOnly>(
                name: "fecha_ingreso_equipo",
                table: "equipos",
                type: "date",
                nullable: false,
                defaultValue: new DateOnly(2026, 6, 17),
                oldClrType: typeof(DateOnly),
                oldType: "date",
                oldDefaultValue: new DateOnly(2026, 8, 25));

            migrationBuilder.AddColumn<int>(
                name: "tiempo_max_prestamo",
                table: "equipos",
                type: "integer",
                nullable: true);

            migrationBuilder.CreateIndex(
                name: "IX_audit_logs_admin_carnet_timestamp",
                table: "audit_logs",
                columns: new[] { "admin_carnet", "timestamp" });

            migrationBuilder.CreateIndex(
                name: "IX_audit_logs_entidad_entidad_id",
                table: "audit_logs",
                columns: new[] { "entidad", "entidad_id" });
        }
    }
}
